...
def from_location_callback(update: Update, context: CallbackContext):
    ...
    if update.message.location:
        ...
        if user['lang'] == LANGS[0]:
            error_text = '\U000026A0 Geolokatsiya yuborilmadi !\n\n' \
                         'Geolokatsiyangizni yuboring yoki «next» ni bosing.'

        if user['lang'] == LANGS[1]:
            error_text = '\U000026A0 Геолокация не отправлена !\n\n' \
...
def cargo_weight_callback(update: Update, context: CallbackContext):
    ...
    if not text.isdigit():
        ...
        if user['lang'] == LANGS[0]:
            text = '\U000026A0 Yuk og\'irligini raqamda kiriting !'

        if user['lang'] == LANGS[1]:
            text = '\U000026A0 Введите вес груза цифрами !'
...
def cargo_volume_callback(update: Update, context: CallbackContext):
    ...
    if not text.isdigit():
        ...
        if user['lang'] == LANGS[0]:
            error_text = '\U000026A0 Yuk hajmini raqamda kiriting !\n\n' \
                         'Yoki bu bosqichni o\'tkazib yuborish uchun «next» ni bosing.'

        if user['lang'] == LANGS[1]:
            error_text = '\U000026A0 Введите объем груза цифрами !\n' \
                         'Или нажмите «next», чтобы пропустить этот шаг.'
...
def cargo_definition_callback(update: Update, context: CallbackContext):
    ...
    if text == '/cancel':
        ...
        if user['lang'] == LANGS[0]:
            text = '\U0000274C E\'lon bekor qilindi !'

        if user['lang'] == LANGS[1]:
            text = '\U0000274C Объявление отменено !'
    ...
    if text == '/menu' or text == '/start':
        ...
        if user['lang'] == LANGS[0]:
            text = '\U000026A0 Sizda tugallanmagan e\'lon mavjud !\n\n' \
                   'E\'lonni bekor qilish uchun /cancel ni yuboring'

        if user['lang'] == LANGS[1]:
            text = '\U000026A0 У вас есть незаконченное объявление.\n\n' \
...
def cargo_photo_callback(update: Update, context: CallbackContext):
    ...
    if len(cargo_photo) > 0:
        ...
        if user['lang'] == LANGS[0]:
            error_text = '\U000026A0 Yuk rasmi yuborilmadi !\n' \
                         'Yuk rasmini yuboring:\n\n' \
                         'Yoki bu bosqichni o\'tkazib yuborish uchun «next» ni bosing.'

        if user['lang'] == LANGS[1]:
            error_text = '\U000026A0 Фотография груза не отправлено !\n' \
...
def receiver_callback(update: Update, context: CallbackContext):
    ...
    if phone_number and phone_number != user['phone_number']:
        ...
        if user['lang'] == LANGS[0]:
            ...
            reply_text += '\nYoki bu bosqichni o\'tkazib yuborish uchun «next» ni bosing.'
            error_text = '\U000026A0 Siz o\'z telefon raqmingizni yubordingiz !\n\n' \
                         'Yukni qabul qiluvchining telefon raqamini yuboring:'

        if user['lang'] == LANGS[1]:
            text = '\U000026A0 Номер телефона с ошибкой отправлен !'
            reply_text += '\nИли нажмите «next», чтобы пропустить этот шаг.'
            error_text = '\U000026A0 Вы отправили свой номер телефона !\n\n' \
...
def confirmation_callback(update: Update, context: CallbackContext):
    ...
    if data == 'confirm':
        ...
        if user['lang'] == LANGS[0]:
            text = '\U00002705 E\'lon tasdiqlandi !'

        if user['lang'] == LANGS[1]:
            text = '\U00002705 Объявление подтверждено !'
...
def txt_callback(update: Update, context: CallbackContext):
    ...
    if user_input_data['state'] == CONFIRMATION:
        ...
        if text == '/cancel':
            ...
            inlinekeyboard = InlineKeyboard('main_keyboard', user['lang']).get_keyboard()

            context.bot.edit_message_reply_markup(update.effective_chat.id, user_input_data.pop('message_id'))
            update.message.reply_text(text, reply_markup=inlinekeyboard)
...
def skip_callback(update: Update, context: CallbackContext):
    ...
    if data == 'skip_to_location':
        ...
        callback_query.edit_message_text(reply_text, reply_markup=inline_keyboard)
    ...
    if data == 'skip_volume':
        ...
        callback_query.edit_message_text(reply_text, reply_markup=get_skip_keyboard(state))
